      ...
      <div class="flex justify-between items-center">
        <div>
          ...
          <p class="text-gray-600 dark:text-gray-400 mt-1">
            Escolha um projeto para criar sua instância personalizada
          </p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Card>
            <CardContent class="p-4">
              <div class="flex items-center justify-between">
                <div>
                  <p class="text-sm font-medium text-muted-foreground">Já Criados</p>
                  <p class="text-2xl font-bold">{{ projetosComInstancia?.length || 0 }}</p>
                </div>
          ...
          >
            ...
            <CardContent class="space-y-4">
              ...
              <div class="flex justify-between items-center pt-2 border-t">
                <div class="flex items-center gap-2">
                  <div 
                    v-if="isProjectCriado(projeto.id)"
                    class="flex items-center gap-1 text-green-600"
                  >
                    <CheckCircle class="w-4 h-4" />
                    <span class="text-xs font-medium">Já criado</span>
                  </div>
                  <div 
                    v-else
                ...
                <div class="flex gap-2">
                  ...
                  <Button 
                    v-if="!isProjectCriado(projeto.id)"
                    size="sm"
                    @click="$inertia.visit(route('meus-projetos.create', projeto.id))"
                  >
        ...
        <Card v-else>
          <CardContent class="text-center py-16">
            ...
            <p class="text-muted-foreground mb-6">
              Não há projetos disponíveis para criar instâncias no momento.
            </p>
            <Button @click="$inertia.visit(route('meus-projetos.index'))">
              <ArrowLeft class="w-4 h-4 mr-2" />
...
import { 
  FolderOpen, ArrowLeft, CheckCircle, Plus, Tag, Clock, Eye, 
  Settings, Circle
...
interface Props {
  projetos: Projeto[]
  projetosComInstancia: number[]
}

const props = withDefaults(defineProps<Props>(), {
  projetos: () => [],
  projetosComInstancia: () => []
})
...
const projetosDisponiveis = computed(() => {
  ...
  const projetosComInstancia = props.projetosComInstancia?.length || 0
  return totalProjetos - projetosComInstancia
})
...
// Methods
const isProjectCriado = (projetoId: number) => {
  if (!props.projetosComInstancia || !Array.isArray(props.projetosComInstancia)) return false
  return props.projetosComInstancia.includes(projetoId)
}

const verProjeto = (projetoId: number) => {
  // Redirecionar para o projeto específico na lista de meus projetos
  window.$inertia.visit(route('meus-projetos.index'), {
    data: { filter_projeto: projetoId }
  })
}
